# StockWinner 智能股票交易系统 v7.9.13

多账户智能股票交易系统 - Agent API + SDK子进程隔离 + 策略沙箱 + 优先级队列 + 系统配置管理 + 信号分配 + MCP服务 + K线增强 + ETF数据支持 + 复权因子管理 + Agent API规范 + 止盈止损同步
...
- ✅ **止盈止损同步机制**（买入成功/配置变更/盘中新高时自动同步）
- ✅ **任务线程隔离**（长时间任务在独立线程执行，避免阻塞事件循环）

### 数据能力
- ✅ PriceCache 内存行情缓存（动态 TTL + source 优先级 + 容量管理 + prev_close 除权计算）
- ✅ 多数据源架构（6 个 Provider 自动切换降级）
- ✅ 因子数据管理（日频 62 字段 + 月频 23 字段）
- ✅ K 线数据（日/周/月，11 种周期，增量同步）
- ✅ 技术指标库（MA/RSI/MACD/BOLL/KDJ/ATR/CCI/ADX）
- ✅ **ETF 数据支持**（K线 + 申赎数据 + 成分股 + 基金份额 + IOPV净值）
- ✅ **ETF 因子计算**（自动识别ETF类型，跳过市值/估值/财务因子）
- ✅ **复权因子管理**（每日盘前定时更新 + h5文件缓存 + 除权涨跌幅计算）
...
| 复权因子更新 | `15 9 * * mon-fri` | 每交易日 9:15 更新除权数据 |
...
### v7.9.13 (2026-06-03)
- [x] 止盈止损同步机制（sl_tp_sync.py + API）
- [x] 资金预检查避免无效委托（订单创建前检查账户+策略资金）
- [x] pending 状态反复触发修复（资金不足时更新 watchlist 状态）
- [x] positions_stop 分类修复（排除 watchlist 股票，避免止损检查遗漏）
- [x] 任务插件线程隔离修复（全部改用 run_manual_xxx 方法）
- [x] CLAUDE.md 新增规则 7：长时间任务线程隔离原则

### v7.9.1 (2026-06-01)
- [x] 复权因子管理优化（定时更新 + h5缓存 + 除权涨跌幅）
- [x] SDK 子进程启动优化（移除启动时 adj_factor 调用）
- [x] prev_close 自动计算（从 h5 文件读取复权因子）
- [x] 监控心跳检查修复（等待自动启动避免误报）
- [x] API 返回 prev_close 字段
...
| 版本 | 日期 | 核心功能 |
|------|------|----------|
| v7.9.1 | 2026-06-01 | 复权因子管理优化、SDK启动优化、除权涨跌幅计算 |
