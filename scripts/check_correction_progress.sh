...
PROGRESS_FILE="/home/bobo/StockWinner/data/correction_progress.json"
LOG_DIR="/home/bobo/StockWinner/logs"

echo "========================================"
echo "stock_daily_factors 数据校正进度"
echo "========================================"
echo ""
...
processed = len(progress.get('processed_stocks', []))
total = 4944  # 总股票数
current = progress.get('current_stock_index', 0)
...
if processed > 0 and start:
    ...
    avg_per_stock = elapsed / processed
    remaining = total - processed
    eta_minutes = remaining * avg_per_stock / 60
    print(f"  预计剩余：{eta_minutes:.1f} 分钟")
    print(f"  平均速度：{avg_per_stock:.2f} 秒/只股票")
EOF
...
# 查找最新日志文件
LATEST_LOG=$(ls -t "$LOG_DIR"/correction_*.log 2>/dev/null | head -1)
