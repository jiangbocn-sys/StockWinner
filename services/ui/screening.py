...
async def get_watchlist(
    ...
    try:
        ...
        if stale_codes:
            ...
            async def _bg_refresh_wl():
                try:
                    ...
                    # 交易时间内注册持续刷新（已废弃 stock_quote_dispatcher，由 monitor 统一刷新）
                    # if can_trade():
                    #     dispatcher.update_subscription(sub_id, stale_codes, interval=60, priority=2)
                except Exception as e:
                    logger.error("watchlist_bg_refresh", f"watchlist 后台刷新失败: {e}")
                    try:
                        gw.unsubscribe(sub_id)
                    except Exception:
                        pass
        ...
        for item in watchlist:
            ...
            if code:
                norm = normalize_stock_code(code)
                if norm in cached_ohlcv:
                    ohlcv = cached_ohlcv[norm]
