...
async def get_strategy_position_stats(
    ...
    for row in rows:
        ...
        if sid and sid in strategy_map:
            ...
            strategy_name = f"策略#{sid}"
            strategy_cash = 0
        else:
            strategy_name = "手动买入"
            strategy_cash = 0

        pos_mv = row["total_mv"] or 0
        strategy_total_asset = strategy_cash + pos_mv  # 策略总资产 = 现金 + 持仓市值
    ...
    return {
        ...
        "available_cash": available_cash,
        "total_strategy_cash": round(total_strategy_cash, 2),  # 策略现金总和
        "cash_warning": total_strategy_cash > available_cash,  # 是否超账户资金
