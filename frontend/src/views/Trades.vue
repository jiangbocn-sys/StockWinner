<template>
  <div class="layout-container">
        ...
        <el-tab-pane label="交易信号" name="signals">
          <el-card>
            ...
            <el-table :data="signals" stripe style="width: 100%">
              ...
              <el-table-column prop="profit_loss" label="盈亏" width="100" align="right">
                <template #default="{ row }">
                  <span :style="{ color: row.profit_loss >= 0 ? '#67C23A' : '#F56C6C' }">
                    {{ row.profit_loss != null ? formatNumber(row.profit_loss) : '-' }}
                  </span>
                </template>
              </el-table-column>
    ...
    <!-- 交易策略创建/编辑对话框 -->
    <el-dialog v-model="strategyDialogVisible" :title="strategyDialogTitle" width="560px">
